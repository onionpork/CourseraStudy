...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
for i = 1:numPeople
    if all(pedigree.parents(i,:) == 0)
        % genotype
%         factorList(i).var = i;
%         factorList(i).card = (nchoosek(numAlleles,2) + numAlleles);
        factorList(i) = genotypeGivenAlleleFreqsFactor(alleleFreqs, i);
        % phenotype
%         factorList(numPeople + i).var = [numPeople + i, i];
%         factorList(numPeople + i).card = [2 (nchoosek(numAlleles,2) + numAlleles)];
%         factorList(numPeople + i).val(1:2:end) = alphaList;
%         factorList(numPeople + i).val(2:2:end) = 1 - alphaList;
        factorList(numPeople+i) = phenotypeGivenGenotypeFactor(alphaList, i, numPeople+i);
    else
        % genotype
%          factorList(i).var = [i, pedigree.parents(i,:)];
%          factorList(i).card = ones(1, 3)* (nchoosek(numAlleles,2) + numAlleles);
         factorList(i) = genotypeGivenParentsGenotypesFactor(...
             numAlleles, i, pedigree.parents(i,1), pedigree.parents(i,2));
         % pheotype
         factorList(numPeople+i) = phenotypeGivenGenotypeFactor(alphaList, i, numPeople+i);
