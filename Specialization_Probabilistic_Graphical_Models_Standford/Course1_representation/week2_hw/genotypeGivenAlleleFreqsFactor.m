...
genotypeFactor.card = nchoosek(length(alleleFreqs),2) + length(alleleFreqs); 
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
for i = 1:prod(genotypeFactor.card)
    a_vector= genotypesToAlleles(i,:) ;
    if a_vector(1) == a_vector(2)
        genotypeFactor.val(i) = alleleFreqs(a_vector(1)) * alleleFreqs(a_vector(2));
    else
        genotypeFactor.val(i) = 2* alleleFreqs(a_vector(1)) * alleleFreqs(a_vector(2));
    end
end
