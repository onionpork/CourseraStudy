...
phenotypeFactor.card = [2, ...
    length(alleleWeights{1})*ones(1, length(geneCopyVarOneList)) , ...
     length(alleleWeights{1})*ones(1, length(geneCopyVarTwoList)) ];
phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));

allT = [];
for j = 1:length(alleleWeights)
    allT = [allT alleleWeights{j}'];
end

Gnum = length(geneCopyVarOneList);
anum = length(alleleWeights{1});
for i = 1:prod(phenotypeFactor.card)
    assignment = IndexToAssignment(i, phenotypeFactor.card);
    
    G = zeros(length(geneCopyVarOneList), length(alleleWeights));
    for k = 1:anum
        for p = 1:Gnum
            idx = anum*(p-1) + k +1; 
            G(k,assignment(idx)) = G(k,assignment(idx)) + 1;
        end
    end
    
    res = reshape(G', [], 1)' * reshape(allT, [], 1);  
    
    if assignment(1) == 1
        phenotypeFactor.val(i) = computeSigmoid(res);
    else
        phenotypeFactor.val(i) = 1- computeSigmoid(res);
    end
end
