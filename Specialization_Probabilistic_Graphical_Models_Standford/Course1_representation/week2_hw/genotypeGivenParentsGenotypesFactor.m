...
for i = 1:prod(genotypeFactor.card)
    ...
    p1G  = assignment(2);
    p2G = assignment(3);
    childA = genotypesToAlleles(childG,:);
    p1A = genotypesToAlleles(p1G,:);
    p2A = genotypesToAlleles(p2G,:);
    cA1 = childA(1);
    cA2 = childA(2);
    p1A1 = p1A(1);
    p1A2 = p1A(2);
    ...
    if cA1 == cA2
        tmp1 = find([p1A1, p1A2] == cA1);
        tmp2 = find([p2A1, p2A2] == cA2);
        genotypeFactor.val(i) = length(tmp1)/ 2 * length(tmp2)/2;
        
    else
        tmp1 = find([p1A1, p1A2] == cA1);
        tmp2 = find([p2A1, p2A2] == cA2);
        op1 = length(tmp1)/ 2 * length(tmp2)/2;
        
        
        tmp3 = find([p1A1, p1A2] == cA2);
        tmp4 = find([p2A1, p2A2] == cA1);
        op2 = length(tmp3)/ 2 * length(tmp4)/2;
        genotypeFactor.val(i) = op1 + op2;
    end
end
